 ...
 * this.onEnterFrame = Delegate.create( this, enterFrameHandler );
 * </listing>
 *
...
class sandy.core.Scene3D
{
	 ...
	 */
	public var materialManager:MaterialManager = new MaterialM
	...
	public function render( p_oEvt:SandyEvent ):Void
	{
		...
		{
			_oEB.broadcastEvent.apply( new SandyEvent( SandyEvent.SCENE_UPDATE ) );
			root.update( this, null, false );
			// --
			_oEB.broadcastEvent.apply( new SandyEvent( SandyEvent.SCENE_CULL ) );
			root.cull( this, camera.frustrum, camera.invModelMatrix, camera.changed );
			// --
			_oEB.broadcastEvent.apply( new SandyEvent( SandyEvent.SCENE_RENDER ) );
			root.render( this, camera );
			// -- clear the polygon's container and the projection vertices list
			_oEB.broadcastEvent.apply( new SandyEvent( SandyEvent.SCENE_RENDER_DISPLAYLIST ) );
	...
	public function set light( l:Light3D ):Void
	{
		...
		// --
		_light = l;
		_oEB.broadcastEvent.apply( new SandyEvent( SandyEvent.LIGHT_ADDED ) );
	}
