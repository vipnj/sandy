...
class sandy.extrusion.data.Polygon2D
{
	 ...
	 * @see #vertices
	 */
	public function Polygon2D( points:Array )
	{
	...
	{
		var a:Number = 0, n:Number = vertices.length;
	 ...
	 */
	public function edges( reorient:Boolean ) : Array
	{
	 ...
	 */
	public function hitTest( point:Point, includeVertices:Boolean ) : Boolean
	{ 
		if( includeVertices == null ) includeVertices = true;
		// first, loop through all vertices
		var i:Number, n:Number = vertices.length;
		for( i = 0; i < n; i++ )
		...
		var crossing:Number = 0; n = V.length - 1;
		for( i = 0; i < n; i++ )
		{
	...
	{
		var ok:Boolean, i:Number, n:Number;
		do
		{
			ok = true;
			n = vertices.length;
	...
	public function triangles() : Array
	{
		...
		var o:Number = area();
		var i:Number = 0, j:Number, k:Number, n:Number, m:Number;
		var ok:Boolean;
	...
	public function bbox() : Rectangle
	{
		...
		{
			if( vertices[ p ].x < xmin ) xmin = vertices[ p ].x;
			if( vertices[ p ].x > xmax ) xmax = vertices[ p ].x;
			if( vertices[ p ].y < ymin ) ymin = vertices[ p ].y;
			if( vertices[ p ].y > ymax ) ymax = vertices[ p ].y;
		}
