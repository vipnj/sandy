...
class sandy.parser.ColladaParser extends AParser implements IParser
{
	...
	private function parseData( s:Boolean ) : Void
	{
		...
		if( m_oCollada.library_images.image ) //There are images
	...
	private function parseScene( p_oScene:Object ) : Void
	{
		...
		for( var i:Number = 0; i < l_nNodeLen; i++ ) // -- Walk through al the nodes of the virtual scene
		{
	 ...
	 */
	private function parseNode( p_oNode:Object /* Don't change this into XMLNode! */ ) : Node
	{
		// -- local variables
		...
		// quick hack to get url-ed nodes parsed, --  need some help with porting
		/*
		if( convertToArray( p_oNode.instance_node ).length() != 0 )
		{
			...
			if ((l_sNodeId != "") && (l_sNodeId.charAt(0) == "#"))
			{
				...
				var l_oMatchingNodes:XMLList = m_oCollada.library_nodes.node.( attributes.id == l_sNodeId );
	...
	private function getGeometry( p_sGeometryID:String, p_oMaterials:Object ) : Geometry3D
	{
		...
		var l_sVerticesID : String = l_oTriangles.input.( attributes.semantic == "VERTEX" ).attributes.source.split("#")[1];
	...
	{
		var l_aValues:Array = p_sValues.split( ' ' ); // TODO content.split( ' ' ) will cause bugs?
	...
	private function getAppearance( p_oNode:Object ) : Appearance
	{
		...
		l_oInstMats = convertToArray( p_oNode.instance_geometry.bind_material.technique_common.instance_material );
		for( var l_oInstMat:Object in l_oInstMats )
		{
