...
class sandy.core.scenegraph.ATransformable extends Node
{
	...
	*/
	...
	{
		trace("RotateY::"+"nAngle:"+nAngle);
		var l_nAngle:Number = (nAngle + 360)%360 - _vRotation.y;
		trace( " RotateY :"+l_nAngle+" _vRotation.y:"+_vRotation.y);
		trace("_vSide avant : "+_vSide);
		// --
		var m:Matrix4 = Matrix4Math.rotationY ( l_nAngle );
		this._vUp   = Matrix4Math.vectorMult3x3( m, this._vUp  );
		this._vSide = Matrix4Math.vectorMult3x3( m, this._vSide);
		this._vOut  = Matrix4Math.vectorMult3x3( m, this._vOut );
		// --
		_vRotation.y = nAngle;
		trace( "_vSide apres : "+_vSide);
		trace("sauvegarde : "+_vRotation.y);
		trace("side vu par le workd3D : "+World3D.getInstance().camera.side);
	...
	{
		trace("Get rotateY:"+_vRotation.y);
		return _vRotation.y;
	}
