...
package sandy.util {
	...
	**/
	...
	public class BezierUtil 
	{
		...
		* @return
		*/
		public static function casteljau_interval( p:Number, plist:Array, pdeb:int, pfin:int ):Vector
		{
			var aNewList:Array = plist.slice();
			// --
			for( var i:int = pdeb; i < pfin; i++)
			{
				var loc_p:int = i % ( plist.length );
				if ( p < 0 ) 
					loc_p += plist.length;
				// --
				aNewList.push( plist[int(loc_p)] );
			}
			// --
