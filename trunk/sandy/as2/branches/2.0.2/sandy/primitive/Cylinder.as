...
class sandy.primitive.Cylinder extends Shape3D implements Primitive3D
{
	 ...
	 */
	function Cylinder( p_sName:String, p_nRadius:Number, p_nHeight:Number,
	                   p_nSegmentsW:Number, p_nSegmentsH:Number, p_nTopRadius:Number,
	                   p_bExcludeBottom:Boolean, p_bExludeTop:Boolean,
	                   p_bWholeMapping:Boolean )
	{
		super( p_sName );
		
		p_nRadius = p_nRadius||100;
		p_nHeight = p_nHeight||100;
		...
		topRadius = ( isNaN( p_nTopRadius ) or !p_nTopRadius ) ? radius : p_nTopRadius;

		var scale :Number = DEFAULT_SCALE;
		m_bIsBottomExcluded = p_bExcludeBottom||false;
		m_bIsTopExcluded = p_bExludeTop||false;
		m_bIsWholeMappingEnabled = p_bWholeMapping||true;
	 ...
	 * @return	The radius
	 */
	public static function CALCUL_RADIUS_FROM_SIDE( p_nSideNumber : Number, p_nSideWidth:Number ) : Number
	{
		 ...
		 * 	or 2r = s * ( 1 / sin 180/n )
		 */
		return ( int( p_nSideWidth ) / ( 2 * Math.sin( Math.PI / int( p_nSideNumber ) ) ) );
	}
