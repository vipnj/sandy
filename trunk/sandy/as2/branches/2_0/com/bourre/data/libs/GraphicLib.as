...
class com.bourre.data.libs.GraphicLib extends AbstractLib
{
	...
	{
		if ( !(GraphicLibLocator.getInstance().isRegistered(sName)) )
		{
			if ( (GraphicLibLocator.getInstance().isRegistered(_sName)) ) GraphicLibLocator.getInstance().unregister(_sName);
			GraphicLibLocator.getInstance().register( sName, this );
			super.setName( sName );
		}
	}*/
